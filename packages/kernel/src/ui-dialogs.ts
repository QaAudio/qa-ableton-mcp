...
export type ConfirmOptions = {
  title?: string;
  /** Primary body text (required unless `message` is set). */
  summary?: string;
  /** Alias for `summary` — common mistake from other dialog APIs. */
  message?: string;
  items?: string[];
  danger?: boolean;
  approveLabel?: string;
...
export type NormalizedConfirmOptions = ConfirmOptions & { summary: string };

/** Resolve `summary` / `message` and require at least one non-empty body string. */
export function normalizeConfirmOptions(opts: ConfirmOptions): NormalizedConfirmOptions {
  const summary = opts.summary ?? opts.message;
  if (summary == null || String(summary).trim() === "") {
    throw new Error("ui.confirm requires `summary` (or `message`)");
  }
  return { ...opts, summary: String(summary) };
}
...
export function buildConfirmHtml(opts: NormalizedConfirmOptions): string {
...
export async function confirmDialog(ctx: Ctx, opts: ConfirmOptions): Promise<boolean> {
  ...
  kernelLog.info(
    "ui",
    `confirm open: ${JSON.stringify(normalized.summary.slice(0, 80))}${normalized.danger ? " [danger]" : ""}`,
  );
  const html = buildConfirmHtml(normalized);
  const url = `data:text/html,${encodeURIComponent(html)}`;
  const h = Math.min(420, 150 + (normalized.items?.length ?? 0) * 22);
  const raw = await ctx.ui.showModalDialog(url, 420, h);
  try {
    const approved = JSON.parse(raw).approved === true;
    kernelLog.info("ui", `confirm ${approved ? "approved" : "cancelled"}`);
    return approved;
  } catch {
    kernelLog.warn("ui", "confirm dialog returned unparseable result — treating as cancelled");
    return false;
  }
}
...
export function progressDialog(
  ...
  kernelLog.info("ui", `progress open: ${JSON.stringify(text.slice(0, 80))}`);
  return ctx.ui.withinProgressDialog(text, { progress: 0 }, async (update, signal) => {
    try {
      const result = await cb(update, signal);
      kernelLog.info("ui", "progress completed");
      return result;
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      kernelLog.warn("ui", `progress failed: ${msg}`);
      throw e;
    }
  });
}
