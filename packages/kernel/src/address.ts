...
import type { Address } from "@quantumaudio/ableton-mcp-schemas";
 ...
 * The shape is shared with the MCP server via `@quantumaudio/ableton-mcp-schemas`
 * (type-only import — not bundled).
 */
export type { Address };
  ...
  switch (addr.kind) {
    case "track": {
      ...
      if (addr.name !== undefined && t.name !== addr.name) {
        kernelLog.warn(
          "address",
          `drift: track ${addr.index} is "${t.name}", expected "${addr.name}" — re-run get_context`,
        );
      }
      return t;
    }
