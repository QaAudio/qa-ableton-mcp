...
ws.on("open", async () => {
  try {
    ...
    // 8b. Unawaited promise rejection must be reported (not "ok" + host-side
    //     unhandledRejection) — the classic `run()` instead of `await run()`.
    r = await run(`
      async function runIt() { throw new Error("floating-boom"); }
      runIt();
      return "done";
    `);
    check(
      "unawaited rejection reported to client",
      !r.ok && r.phase === "runtime" && /unawaited promise rejected/.test(r.error ?? "") && /floating-boom/.test(r.error ?? ""),
      `ok=${r.ok} phase=${r.phase} err=${(r.error ?? "").split("\n")[0]}`,
    );

    // 8c. `ableton.withinTransaction` (namespace mistake) gets a steering error.
    r = await run(`return ableton.withinTransaction(() => 1);`);
    check(
      "ableton.withinTransaction steering error",
      !r.ok && /does not exist/.test(r.error ?? "") && /withinTransaction\(fn\)/.test(r.error ?? ""),
      `ok=${r.ok} err=${(r.error ?? "").split("\n")[0]}`,
    );
